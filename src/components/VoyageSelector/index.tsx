...
const VoyageSelector: React.FC<VoyageSelectorProps> = ({ visible, onClose, mode = 'all' }) => {
  ...
  const [step, setStep] = useState<'ship' | 'voyage'>(mode === 'ship' ? 'ship' : mode === 'voyage' ? 'voyage' : 'ship')

  const filteredVoyages = selectedShipId
    ? voyages.filter(v => {
        const ship = ships.find(s => s.id === selectedShipId)
        return ship && v.shipName === ship.name
      })
    : voyages
  ...
  const handleSelectShip = (ship: Ship) => {
    ...
    if (mode === 'ship') {
      ...
      Taro.showToast({ title: `已切换至${ship.name}`, icon: 'success' })
        .catch(() => {})
      onClose()
    } else {
      setStep('voyage')
  ...
  const handleSelectVoyage = (voyage: Voyage) => {
    ...
    Taro.showToast({ title: `已切换航次`, icon: 'success' })
      .catch(() => {})
    onClose()
  }
      ...
      <View className={styles.sheet} onClick={e => e.stopPropagation()}>
        ...
        {step === 'voyage' && (
          <View className={styles.backBar} onClick={() => setStep('ship')}>
            <Text className={styles.backText}>← 返回选择船舶</Text>
          </View>
        )}

        <ScrollView scrollY className={styles.listContainer}>
          {step === 'ship' && ships.map(ship => (
            <View
              key={ship.id}
            ...
            >
              <View className={styles.itemMain}>
                ...
                <Text className={styles.itemSub}>{ship.type} · {ship.loadWeight}吨</Text>
              </View>
            ...
            >
              <View className={styles.itemMain}>
                ...
                <Text className={styles.itemSub}>
                  {voyage.startDate} 出发 · {voyage.distance}海里
                </Text>
              </View>
